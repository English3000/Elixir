...
defmodule PlateSlate.Menu do
  ...
  def query(Item, filters), do: items_query(filters)
  ...
  def categories_by_id(_, ids) do
    Category |> where([c], c.id in ^Enum.uniq(ids))
      |> Repo.all
      |> Map.new(fn categ -> {categ.id, categ} end)
  end
  ...
  defp filter_by(query, filter) do
    Enum.reduce(filter, query, fn
      ...
      {:category, category_name}, query -> from q in query,
                                           join: c in assoc(q, :category),
                                           where: ilike(c.name, ^"%#{category_name}%")
      {:tag, tag_name}, query -> from q in query, join: t in assoc(q, :tags),
  ...
  defp search_ecto(schema, pattern) do
    Repo.all from q in schema, where: ilike(q.name, ^pattern) or
